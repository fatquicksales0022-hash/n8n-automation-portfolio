...
|---------|-----------------------|-------|
| [gigradar](./gigradar/) | **Three job boards → one ranked feed → drafted proposals.** Scrapes OnlineJobs.ph, LinkedIn, and Upwork daily, normalizes them into one canonical record, and **dedupes before the AI** so seen jobs never cost a call. **AI-scores** every listing 0–100 against a skill profile (hard-gap tools land in `red_flags`), logs every score, and for the ≥70 winners **drafts an honest cover letter** — bound to only claim shipped work — posted to Slack for review. Never auto-applies. | n8n · Apify · GPT-4o-mini · Google Sheets · Slack |
...
| [hr-hireflow](./hr-hireflow/) · [▶ Demo](https://www.loom.com/share/d8cd666fdcd94a1980e687b8ac700dc5) | **Résumés in, hired-and-onboarded out — a human in every loop.** Five workflows: AI-screens each applicant against a data-driven Airtable scorecard (job-relevant evidence only, full reasoning logged as an audit trail), posts a Slack review card, and on one human click **drafts** the interview invite or rejection — with **no auto-reject path anywhere**. Then onboarding runs itself: role-specific checklist, welcome draft, Day-1 calendar event, a daily document chaser, and a weekly HR brief. Built for hiring-AI compliance (NYC LL144 / EU AI Act): **AI recommends, a human decides.** | n8n · GPT-4o-mini · Airtable · Gmail · Slack · Google Calendar |
...
| [shared-error-handler](./shared-error-handler/) | **The production-readiness layer.** One 3-node workflow that every other build points at — the moment *any* workflow fails, it formats the error (which workflow, which node, the message, a deep link to the failed run) and emails an alert instantly. Wired into every workflow in one pass via the n8n public API, so a silent failure becomes an inbox alert you can act on in seconds. | n8n · Gmail |
